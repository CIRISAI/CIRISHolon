...
## F-? — the 3D lane's bridge is a gate without its implementation (2026-09-03, opened by the lead)

**State, measured, not inferred.** `docs/workbench/smoke.mjs` has carried three checks since
`7140793` (2026-09-02 16:47) requiring that `docs/atoms3d/index.html` call
`wasm.holon3d_run_owned()`, guard the call with a `typeof` check, and that
`engine/crates/holon-render-3d/build-web.sh` run `check-exports.js` on the finished
artifact. **The committed tree satisfies none of them** — 0 occurrences of the entry point
in the committed page, no `check-exports.js` invocation in the committed build script, and
`check-exports.js`, `src/bridge.rs` and `tests/bridge.rs` are UNTRACKED. The shared working
tree does satisfy them, so every local run of the gate passes and CI fails: on a clean
worktree at `73a9ccd` the workbench gate reads **373 of 376**, with exactly these three red.

**This is the shape the gate exists to catch, one level up.** The gate was committed and the
code it gates was not, so main's 3D page loads its module and draws nothing, and the only
thing that made this invisible was a dirty shared tree. It is also why `verify` has been red
since 16:47 — before the timeout raise and before the thick-restart work, both of which are
