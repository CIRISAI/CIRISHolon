...
|---|---|---|
| **EDGE-0 — READ (freeze 151fc79; `EDGE0_RESULTS.md`)**: the cluster carrier has TWO DENSITIES and NO EDGE | the pair rule with two arms was measured anti-cohesive (5.9M of 6.3M releases a particle claimed at another mover), so the rule the grammar states was added: the mover per connected component, the closure moving or waiting AS ONE, the rent clause handed the bond's lifetime back (116k of 211k releases by rent against 1.8k of 6.3M). Coexistence C (a), real and the cluster rule's alone — but DISPERSED: slab and droplet scenes evaporate in 100 steps, so Laplace, the width and the two-σ agreement read on scenes that did not hold (L (b), W (b), A vacuous), gravity settled nothing (G1B (b)); the droplet's lifetime is the number (D1 (b)). Instrument findings kept: the dip cannot gate a minority phase; a zero control band voids; C (a) shares its seeds with the pass that chose the chart | the next cohesion is an ATTRACTION, not a constraint on displacement: Appert–Zaleski's non-local momentum exchange on this carrier, priced before EDGE-0 is repeated; the six instruments and the carrier's four rules stand |
