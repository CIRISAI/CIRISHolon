...
| fluid element | long-wavelength density and current modes | **the STAGGERED chart — density in cells, momentum on the faces within `0.25 Å`** (`D = 0.164` on the 200 m/s control at 8 cells); the cell chart `(n̄, p̄)` at `70–78 %` of the bound leaks a third of its signal at one-diameter slabs | the momentum density's molecular layering (transfer `0.45` at one diameter, `0.2` at half); single-particle velocity memory (0.3–0.4 of the bound, carried by no field) | **In the engine (REPLACE-1, 2026-09-26): the staggered chart and the cell chart behind one interface (`holon-lens::staggered`), the tier admitted by ORDERED removability (`holon-closure::removable`) — carried = not removable within budget, with a price certificate; the engine reproduces R1″'s `0.1800`, ORDER-1's eighteen increments and ACUITY-1's 76 removed gates to the digit; the hydrodynamic block survives alone, the structural block is dropped at `≤ 0.003`.**
...
| **hadron — colour-singlet closure (the fold below the atom)** | **NAMED AND LOCKED 2026-09-02 as a FOLD, wager strength, three separable kills** (`OBJECT.md` "The fold below the atom"): Gauss's law is the seam — the hadron tier closes on an EXACT conservation fiber (colour), the far field is free by the mass gap, and the one priced object is a hadron in its own grain, priced by the strata this engine owns | GF0 SCHWINGER-4 (residual interaction between screened static pairs decays at the banked M_V; prereg frozen alone 2026-09-02), GF1 the magic price of Z₂/Z₃ gauge vacua on the exact tiers, GF2 the Σ(1080) hadron box against p/n/π and the deuteron | **first rung MEASURED on the engine arm (2026-09-02): SCHWINGER-4 G1 branch (a) on both columns, the residual interaction between two screened pairs decaying at the banked meson mass to 0.6% (`SCHWINGER4_RESULTS.md`; Python cross-check pending per amendment A1)** — Fold II's first measurement below the atom, 1+1D only **2026-09-02, GF2a run:** the EXACT arm (colour lanes, one string per conserved colour count — the swarm's integer-lane law at this tier — host and device bit-identical) reaches N = 10 on both couplings; the MPS arm is CONVICTED at χ = 64 by G0 (penalty-sector metastability, not truncation) and retired for this rung; G1–G3 unread, N ≤ 10 is below the volume standard and the frozen G1 ratio is one 1/N data cannot meet — the rung stays OPEN, successor E7 (U(1)³ MPS blocks). `conformance/crystal/GF2A_QCD2_RESULTS.md`.
