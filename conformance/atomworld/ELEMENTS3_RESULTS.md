...
what the refusals mean, not what any gate concluded — and it is the one finding here that
changed the engine rather than only the record: a crate-wide measurement followed it, found
`CONVERGED_RESIDUAL` sitting exactly on the solver's expansion floor, and the bar is now derived
a decade above that floor instead of coinciding with it. Four of this record's five refusals
would not be refused under the replacement. The printed verdicts are not rewritten, because no
energy moved and a record is what an instrument produced under a declared rule, not a
current-best-state.*
...
| 5 | the R1 record printed `sol.residual` and **checked nothing**, so indium — residual **3.98e-1** against ~1e-10 elsewhere — appeared as a row indistinguishable from a measurement. (This entry said "Davidson stopped at its 1200-iteration cap" until the exit reasons were measured; every other atom turned out to stagnate far short of its cap, and indium's own exit is still unmeasured. The correction is in the body, and it had survived here for the reason ledgers usually outlive their corrections: nobody re-reads a table when they fix a paragraph.) Applying the fix then caught **zinc** too, at 2.72e-10, a number this document had already published | the crate had ALREADY written `pair::CONVERGED_RESIDUAL` for exactly this, with a doc comment describing it word for word; I did not reach for it | the row is REFUSED on two independent checks — the declared residual bar, **and** 2S+1 integrality, which is free and catches the failure for a reader who never looks at the residual column |
