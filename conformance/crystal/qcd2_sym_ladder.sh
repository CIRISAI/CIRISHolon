...
# χ-ladder 64 → 128 → 256 in one process (the 32 rung dropped: at N = 8 the middle bond carries 64 charge labels); one JSON per point; resumable.
...
printf '%s\n' "${jobs[@]}" | xargs -P "${QCD2_WORKERS:-6}" -L 1 bash -c '
  ...
  Q8_THREADS='"${Q8_THREADS:-4}"' '"$BIN"' --n $n --x $x --b $b --chi 64,128,256 --sweeps 60 --sym > "$out.tmp" && mv "$out.tmp" "$out" && echo "$out $(cat "$out")" || echo "FAILED $x $n $b"
'
echo LADDER_DONE
