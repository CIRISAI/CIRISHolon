...
TRIPLES = list(_comb(range(6), 3))

def triple_vector(st, p, q, r):
    n2 = norm2(*st)
    hp, hq, hr = hol_of(p), hol_of(q), hol_of(r)
    return normalize(tuple(int(np.sum(n2[:, (hp == a) & (hq == b) & (hr == c)]))
                           for a in range(3) for b in range(3) for c in range(3)))

def v_triple(st):
    return v_pair(st) + tuple(triple_vector(st, p, q, r) for p, q, r in TRIPLES)
...
def run_c4():
    ...
    rep["G0"] = "PASS" if (nonzero(x) and W.gauss_holds(x)[0]) else "FIRE"
    traj = [x]
    vc, vt = [v_conf(x)], [v_triple(x)]
    tot = [normalize(triple(x, W.HOL_LOOP * 0))]
    for k in range(1, 9):
        x = step(x)
        traj.append(x)
        vc.append(v_conf(x)); vt.append(v_triple(x))
        tot.append(normalize(triple(x, W.HOL_LOOP * 0)))
    firing = [(i, j) for i in range(8) for j in range(i + 1, 8)
              if vc[i] == vc[j] and vc[i + 1] != vc[j + 1]]
    rep["K1"] = ("PASS" if firing == [(1, 5), (2, 4), (5, 7)] else f"FIRE got {firing}")
    own = [(i, j) for i in range(8) for j in range(i + 1, 8)
           if vt[i] == vt[j] and vt[i + 1] != vt[j + 1]]
    rep["Q2"] = ("BRANCH(a): v_triple closed on the trajectory -- memory EXACTLY third-order"
                 if not own else
                 f"BRANCH(b): v_triple has OWN firing collisions {own} -- memory beyond third order")
    drift = [k for k in range(1, 9) if tot[k] != tot[0]]
    rep["K3"] = "PASS" if not drift else f"FIRE {drift}"
    rep["B3"] = "PASS" if all(W.gauss_holds(s)[0] for s in traj) else "FIRE"
    print("GATES: " + "  ".join(f"{k}={v}" for k, v in sorted(rep.items())), flush=True)
...
def plants_c4(traj, vt):
    ok = True
    # (i)' 4B: the measured (1,7) pair-collision is the carrier; the triple
    # channel must separate it. Sector: the triple difference, nonzero.
    x1, x7 = traj[1], traj[7]
    pair_equal = v_pair(x1) == v_pair(x7)
    tri_diff = vt[1] != vt[7]
    print(f"[plant i] measured (1,7) collision: pair equal={pair_equal}, "
          f"triple separates={tri_diff} -> {'FIRES' if pair_equal and tri_diff else 'MISSED'}")
    ok &= pair_equal and tri_diff
    x1s = step(traj[0])
    raw0 = triple(x1s, W.HOL_LOOP * 0)
    rawm = triple((x1s[0] * 2, x1s[1] * 2), W.HOL_LOOP * 0)
    fired = rawm != raw0
    print(f"[plant ii] mis-scale -> {'FIRES' if fired else 'MISSED'}")
    ok &= fired
    return ok

if __name__ == "__main__" and "--c4" in sys.argv:
    rep, traj, vt = run_c4()
    ok = plants_c4(traj, vt)
    hard = all(not str(v).startswith("FIRE") for v in rep.values())
    print(f"\nVERDICT: {'no gate fired' if hard else 'FIRED'}; Q2={rep['Q2'][:56]}; "
