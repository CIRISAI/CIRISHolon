...
def off_channel_vacuum():
    """LOCAL-2 U1: live off-channel carrier (dressed channel projected out
    of a generic Gauss-projected state)."""
    rng = np.random.default_rng(11)
    A = rng.integers(-2, 3, size=(9, N)).astype(np.int64)
    B = np.zeros_like(A)
    st = (A, B)
    for v in VERTS:
        aA = np.zeros(st[0].shape, dtype=st[0].dtype); aB = np.zeros_like(aA)
        for g in range(3):
            x, y = gauge_at(st, v, g); aA = aA + x; aB = aB + y
        st = (aA, aB)
    _, off = channel_split(st)
    return off

def matter_weight(st):
    """the dressed-channel weight -- the matter reading for U2."""
    A, B = st
    pA = np.zeros(N, dtype=A.dtype); pB = np.zeros(N, dtype=A.dtype)
    for q in range(3):
        a, b = wpow_mul(A[3 * q + q], B[3 * q + q], (-q * LINE) % 3)
        pA = pA + a; pB = pB + b
    a = pA.astype(object); b = pB.astype(object)
    return int(np.sum(a * a - a * b + b * b))

def run_local2():
    rep = {}
    psi0 = dressed_state()
    rep["G0"] = "PASS" if (nonzero(psi0) and gauss_holds(psi0)[0]) else "FIRE"
    # U1: dressed vs off-channel triples at the matter-disjoint plaquette
    off = off_channel_vacuum()
    if not nonzero(off):
        rep["U1"] = "VOID (off carrier empty)"
    else:
        a, b = psi0, off
        seen = False
        for k in range(1, 5):
            a = step(a); b = step(b)
            ta, tb = triple(a, HOL_P[3]), triple(b, HOL_P[3])
            sa, sb = sum(ta), sum(tb)
    ...
    live, dist = set(), {}
    for k in range(1, 5):
        a2 = step(a2); b2 = step(b2); c2 = step(c2)
        if any(response(a2, c2, P_PEND)):
            live.add(k)
        dist[k] = response(a2, b2, P_PEND)
    rep["U3"] = ("PASS (cone at live steps %s)" % sorted(live)
                 if live and all(not any(dist[k]) for k in live)
                 else ("VOID (no live step)" if not live else "FIRE"))
    rep["B3"] = "PASS" if all(gauss_holds(s)[0] for s in (a, b, a2, b2, c2)) else "FIRE"
    print("GATES: " + "  ".join(f"{k}={v}" for k, v in sorted(rep.items())), flush=True)
    return rep, psi0

if __name__ == "__main__" and "--local2" in sys.argv:
    rep, psi0 = run_local2()
    ok = plants(psi0)
    hard = all(str(v).startswith("PASS") for v in rep.values())
    print(f"\nVERDICT: {'ALL PASS' if hard else 'FIRED/VOID'}; plants {'both FIRE' if ok else 'VOID'}")
