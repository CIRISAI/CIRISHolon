...
def u_e(st):
    """Z3 Fourier kernel on each spoke: psi'(d) = sum_k w^{dk} psi(k).
    Unitary up to 1/sqrt3 per spoke; we carry the integer kernel (global
    3^{-5/2} per step, irrelevant to every gate, all of which compare
    weights)."""
    A, B = st
    for e in range(5):
        A2 = np.zeros_like(A); B2 = np.zeros_like(B)
        for k in range(3):
            ...
            # phase w^{d*k} where d is THIS config's digit
...
def plants(dressed):
    ...
    if t[1] != t[2]:
        flipped = t_conj != t
        print(f"[plant ii] conjugated readout -> {'FIRES (visible)' if flipped else 'MISSED'}")
        ok &= flipped
    else:
        # equal weights at step 0: check on an evolved state where W2 held
        st = step(dressed)
        t = weight_triple(st, HOL_P[P_FAR])
        flipped = (t[0], t[2], t[1]) != t
        print(f"[plant ii] conjugated readout (evolved) -> {'FIRES' if flipped else 'MISSED'} {t}")
        ok &= flipped
    return ok
