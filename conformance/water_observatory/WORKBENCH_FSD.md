...
| WB-10.2 | the atom band's solve in the page: the picked atom's (or its molecule's) STO-3G FCI on the lane engine in wasm; the `holon_atom_band_*` doors: `holon_atom_band_solve(i)` performs it and `holon_atom_band_energy/n_electrons/residual/exit(i)` read the kept result (exit 4 = not computed, never a zero drawn as a digit); `holon_law_probe()` is the fixed reference solve (the oxygen atom) the bit-identity row shows | lead (engine) | `tests/wasm_law.rs` natively computes `holon_law_probe` and pins its bits in `docs/workbench/law_probe.json`; the smoke gate reads the same file: wasm == native TO THE BIT |
