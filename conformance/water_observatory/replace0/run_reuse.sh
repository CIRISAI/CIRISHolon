#!/bin/bash
# REPLACE-0's run 4: the rigid modes kept as projected, the flexible arm re-read with its rigid-mode temperature, the branch reused (2026-09-09): 20,000 settling frames under the stochastic
...
taskset -c 31 ./target/release/examples/replace0 run "$OUT" --settle 20000 --frames 20000 --readouts 40 --refine --reuse > "$OUT/run.log" 2> "$OUT/run.err"; code=$?
