...
OUT="$BASE/response1_fine_T_seed0"
[ -e "$OUT/run.done" ] && { echo "response1-fine: $OUT/run.done exists; REFUSED to overwrite"; exit 5; }
mkdir -p "$OUT"
echo "response1-fine: $(date -Is) arm fine_T seed 0 on cores 0-15, 8 workers"
taskset -c 0-15 "$BIN" scout "$OUT" --cells 6 --settle 3000 --settle-rigid 3000 --readout-fs 10 --stiffness "$STIFF" --seed 0 --workers 8 \
    --kick T --kick-mps 50 --kick-cycles 12 --kick-relax 314 --kick-arm flexible > "$OUT/scout.log" 2> "$OUT/scout.err" &
arm=$!
...
wait "$arm"; c=$?
echo "$c" > "$OUT/run.done"
echo "response1-fine: $(date -Is) arm fine_T seed 0 exit $c"
wait
