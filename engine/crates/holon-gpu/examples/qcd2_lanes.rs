...
//!
//! `gpu` is the DEVICE-RESIDENT solve (tables and vectors on the card, the host holding the
//! m×m eigenproblem); `both` runs every sector on both arms and prints their disagreement
//! (expected: none, to the bit). `--max-sub` is the Davidson subspace bound each arm's door
//! prices (default 48): a space refused at 48 is admitted at 12 with the same driver.
...
use holon_gpu::{solve_lanes_on_device, GpuSigmaProvider};
...
fn main() {
    ...
    for pair in pairs.chunks(2) {
        ...
        for b in 0..=2 {
            ...
            let mut dev_sol = None;
            if let Some(p) = &provider {
                let t0 = Instant::now();
                match solve_lanes_on_device(p.context(), &tables, reserve_mib, None, davidson_budget(), max_sub) {
                    Ok(s) => {
                        line("gpu", &s, t0.elapsed().as_secs_f64());
                        dev_sol = Some(s);
                    }
                    Err(e) => println!("N={n:2} x={x:4.1} B={b} gpu REFUSED: {e}"),
                }
            }
            if cpu {
                let t0 = Instant::now();
