//! THE TRAJECTORY DUMP: the same frozen protocol as `waterquench`, leaving its window
//! behind so the closure census can read it.
//!
//! `waterquench` reports the FINAL FRAME. That is a statement about one instant and about
//! a formula, and `conformance/water_observatory/CENSUS_PREREG.md` exists because the
//! road's item 5 asks a different question: is the product one persistent three-member
//! QUOTIENT, or a component whose composition happens to read like water. Answering it
//! needs the trajectory, so this runner writes one.
//!
//! **Why a second file rather than a flag on the first.** `waterquench.rs` was being
//! edited by another lane while this was written, and `sim.rs` with it — the shared tree
//! did not compile for part of that time. A flag would have meant editing a file in
//! flight. What guards the duplication instead is a mechanical gate:
//! `tests/protocol_identity.rs` extracts the frozen-protocol block from BOTH sources and
//! demands byte equality, so any constant either lane changes fires it. The block below,
//! from the `THE FROZEN PROTOCOL` banner to the end of `reading`, is that block and is
//! copied verbatim; nothing in it may be edited here.
//!
//! **The table set is an ARGUMENT, and there is no default.** `--ozone=served` loads the
//! (O,O,O) surface; `--ozone=fenced` leaves it empty and lets the fence fire on the four
//! OOO triples per seed. The two are different physics and they produce different
//! molecules, so a runner that picked one silently would make its own output
//! unattributable. OBJECT.md's generator discipline is the same rule one tier down: the
//! caller supplies the physics and the layer refuses to invent it.
//!
//! ```text
//! cargo run --release -p holon-render --example waterquench_traj -- \
//!     <arm> --ozone=served|fenced --out=<dir> [seed ...]
//! ```
...
// ================================================================ the run, and the dump

/// Which (O,O,O) treatment this run used. Recorded per run because it is the difference
/// between two physics, not a switch.
#[derive(Clone, Copy, PartialEq)]
enum Ozone {
    Served,
    Fenced,
...
impl Ozone {
    fn parse(s: &str) -> Option<Ozone> {
        match s {
            "served" => Some(Ozone::Served),
            "fenced" => Some(Ozone::Fenced),
            _ => None,
        }
    }
    fn label(self) -> &'static str {
...
/// bits `cluster_species_counts` reads one line later. It does NOT recompute the bond
...
fn run(s: &mut Sim, arm: Arm, seed: u64, out_dir: &Path) -> Outcome {
    let t0 = Instant::now();
    place(s, arm, seed);
    assert!(
        s.pairs_ready(),
    ...
    let path = out_dir.join(format!("seed_{seed:#018x}.traj"));
    // The header is built AFTER `place`, because `place` calls `adopt_table_timescale`
    // and the timestep is a fact about the placed scene. Reading `dt` off the empty box
    // records the fallback -- the very defect `waterquench`'s own comment documents.
    let header = Header {
        seed,
        n_atoms: s.n,
        dims: 2,
    ...
    let mut w = TrajWriter::create(&path, &header).expect("the trajectory file opens");
    let mut pos = vec![[0.0f64; 3]; s.n];
    let mut vel = vec![[0.0f64; 3]; s.n];
    ...
    let frames_written = w.finish().expect("the trajectory closes");
    Outcome {
        seed,
        dt: s.dt(),
        r: reading(s),
...
fn main() {
    ...
    let ozone = args
        .iter()
        .find_map(|a| a.strip_prefix("--ozone="))
        .and_then(Ozone::parse)
        .expect(
    ...
    for &seed in &seeds {
        let o = run(&mut base, arm, seed, &out_dir);
        // The SAME line `waterquench` prints, field for field, so that G2 -- the
        // protocol-equality gate -- is a string comparison against a banked reference log
        ...
        println!(
            "# dumped {} frames to {} ({:.1} MB)",
            o.frames_written,
            o.path.display(),
            std::fs::metadata(&o.path).map(|m| m.len() as f64 / 1e6).unwrap_or(0.0)
        );
    }
}
