...
#[no_mangle]
pub extern "C" fn holon_set_seam(on: u32, a: f64, b: f64, p: f64, c: f64, c6: f64, a_oh: f64, b_oh: f64, a_hh: f64, b_hh: f64, p_hh: f64, c_hh: f64) -> u32 {
    match sim().set_seam(if on != 0 { Some(seam::SeamModel { a, b, p, c, c6, a_oh, b_oh, a_hh, b_hh, p_hh, c_hh }) } else { None }) {
        Ok(()) => 0,
