...
impl<'s, S: Surface + ?Sized> SurfaceSpec<'s, S> {
    ...
    ///
    /// Note how this composes with [`Self::with_device`]: the device class is part of the
    /// REGIME the checkpoint refuses to mix across, so declaring one and replaying the other
    /// is exactly the case `crate::checkpoint`'s regime line exists to catch.
